...
| G4. ChatbotService tool contract | P0 | QuoteEngine channel exposes only customer-safe `quote_*` tools; tool declarations match BFF dispatch; every confirmation-gated write action is scoped to the preparing customer; missing QuoteEngine context fails closed; transport failures return safe tool errors. | `ToolRegistryTests`, `QuoteEngineToolHandlerTests`, `SystemInstructionDefaultPromptTests`, BFF `AgentController` signed context-token requirement; QuoteEngine `Agent_action_confirmation_rejects_another_signed_in_customer_for_pending_and_completed_actions`. | Passed |
...
| G14. Observability and deploy evidence | P1 | Release candidate has named build/test/E2E evidence, clean worktrees, committed run notes, and no unexplained stale service/process blockers. | Final RC evidence recorded in `make-studio-production-readiness-final-report.md`; focused builds/tests across touched repos; Aspire TRX `make-studio-e2e-rc-20260621-after-restore-fix.trx`. | Passed for this release-candidate checkpoint; rerun once after external config freeze. |

## Remaining Work

1. **Before deployment config freeze:** rerun the focused Aspire Make Studio E2E once against the frozen environment/configuration.
2. **Outside this Make Studio gate:** continue separate product gates for email delivery, NDA lifecycle, generic document management, broader customer portal/storefront flows, and non-Make Studio Intranet modules.

...
**S1-extras — ✅ DONE (in `Maliev.QuoteEngine.Bff`):** signed, HttpOnly anonymous-visitor cookie (`AnonymousVisitorCookie`, HMAC mirroring `CustomerAssistantHandoffCookie`) + an ASP.NET rate limiter (`quote-agent` policy) on the cost endpoints (`messages`, `messages/stream`, `clean-speech`). **Partition key is the visitor id only from a *valid inbound* cookie, else client IP** — a freshly minted id never becomes the key, so a cookie-dropping abuser always lands on the IP partition (the real abuse floor); the cookie only buys NAT fairness. `PermitLimit` from `QuoteAgent:RateLimit:PerMinute` (default 30; `0` → `GetNoLimiter`, the disabled state; off by default under Testing). Cookie issuance is a separate middleware (non-security; no ordering dependency with the limiter). 3 new `QuoteAgentRateLimitTests` incl. the **`HandleCookies=false` abuser-path 429** acceptance test. Build 0/0; **+3 passing, 0 regressions** (the QuoteEngine suite has 5 *pre-existing* failures from unrelated in-progress uncommitted work — monochrome launch-shell/CSS redesign, prompt-guidance substrings, artifact-download URL encoding, payment snapshot — confirmed identical with and without these new tests, so untouched here).

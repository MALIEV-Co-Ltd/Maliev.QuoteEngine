...
- **Dead-end error (CONFIRMED via replicad `dist/replicad.d.ts`):** the worker calls `sketch.lineTo(p[0], p[1])` (two scalars) in `Maliev.QuoteEngine.Client/js-src/replicad-worker.js:185,188` and `.lineTo(radiusBottom, 0)` in `makeCone` (`:243`), but replicad's `lineTo(point: Point2D)` takes a **single `[x, y]` array**. replicad receives `point = p[0]` (a scalar such as `25`) and destructures it as a `Point2D`, throwing `number 25 is not iterable`. This means **every line-based silhouette** (including the hardcoded hand keychain) and **every cone** throws. (`threePointsArc(xDist, yDist, viaXDist, viaYDist)` genuinely takes four scalars, so `arc` was already correct — the earlier arc hypothesis was disproved by reading replicad's type definitions.) `ValidateCadCommands` never catches this because the commands are *valid* — the worker's arg-passing is wrong. `RetryPreview()` re-runs identical commands, guaranteeing repeat failure.
